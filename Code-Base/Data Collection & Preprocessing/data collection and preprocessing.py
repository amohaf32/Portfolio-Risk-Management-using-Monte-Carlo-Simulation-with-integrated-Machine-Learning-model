...
# # Split into 'price_type' and 'ticker' (split on first underscore)
# data[['price_type', 'ticker']] = data['column'].str.split('_', n=1, expand=True)
# data = data.drop(columns=['column'])
...
# # Debugging: Print the 'column' series to inspect its contents
# print("Unique values in 'column':", data['column'].unique())

# # Split into 'price_type' and 'ticker' (split on first underscore)
# split_result = data['column'].str.split('_', n=1, expand=True)

# # Check if the split produced exactly two columns
# if split_result.shape[1] != 2:
#     raise ValueError("Split operation did not produce exactly two columns. Check the 'column' values.")

# # Assign the split results to new columns
# data[['price_type', 'ticker']] = split_result
...
#         price_type, ticker = col_tuple
#     else:
#         price_type, ticker = col_tuple, None  # Handle unexpected cases
...
# # Handle potential missing underscores safely
# data[['price_type', 'ticker']] = data['column'].apply(lambda x: x.split('_', 1) if '_' in x else [x, None]).apply(pd.Series)
